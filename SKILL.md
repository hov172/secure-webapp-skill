...
- `$secure-webapp maintain`: update this skill package. Runs in a clone of the source repository, not in an installed copy: `scripts/refresh.py`, then curate `references/` from `_sources/CHANGES.md`, then `scripts/eval_skill.py` (with `--check`), `scripts/check_skill.py`, and `scripts/package_skill.py`.
...
Maintenance sources are in `scripts/manifest.json`, and `scripts/reference_map.json` records which sources ground which reference. Use `scripts/refresh.py`, `scripts/eval_skill.py`, `scripts/check_skill.py`, and `scripts/package_skill.py` when updating or publishing the skill, from a clone of the source repository.
